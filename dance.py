...
def dance(png_dir, gif):
    ...
    if gif in os.listdir('.'):
        os.remove(gif)
    subprocess.Popen('convert -delay 10 -loop 0 %s/*.png %s.gif' % (png_dir, gif), shell=True, stdout=subprocess.PIPE).stdout.read()
...
def dance_all():
    ...
    dirs = [curdir, 'harvest']
    for hewotm in sorted(os.listdir(os.path.sep.join(dirs))):
        dirs.append(hewotm)
        for run in sorted(os.listdir(os.path.sep.join(dirs))):
            dirs.append(run)
            dir_name = os.path.sep.join(dirs)
            print(dir_name)
            if 'done' not in os.listdir(dir_name):
                plot_fig_in_dir(dir_name)
                dance(dir_name, dirs[0]+'/gif/' + '_'.join(dirs[2:]))
