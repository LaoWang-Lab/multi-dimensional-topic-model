...
import sys, urllib2, re, string, time, threading, argparse, os
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n', '--num', type=str, help='the number of articles you want to download')
    parser.add_argument('-d', '--dir', type=str, help='the directory to save articles')
    args = parser.parse_args()

    article_num = int(float(args.num)) if args.num else 1e4
    save_dir = args.dir if args.dir else 'data'
    ...
    for i in range(0, article_num, 2 * MAX_THREADS):
        (articles, articlenames) = get_random_wikipedia_articles(2 * MAX_THREADS)
        for i in range(0, len(articles)):
            print(articlenames[i])
            try:
                with open(os.path.join(save_dir, articlenames[i]+".txt"), 'wt') as f:
                    f.write(articles[i])
            except:
                pass
